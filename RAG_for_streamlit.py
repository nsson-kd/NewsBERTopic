...
def build_hybrid_retriever(files_dict):
    ...
    for period_label, file_paths in files_dict.items():
            ...
            for file_path in file_paths:
                ...
                # 1. Parquet 파일 경로 생성 (.csv 확장자를 .parquet로 변경)
                parquet_path = file_path.replace('.csv', '.parquet')
                
                temp_df = pd.DataFrame() # 안전을 위한 빈 데이터프레임 초기화
                
                # 2. Parquet 파일이 존재하는지 최우선으로 확인
                if os.path.exists(parquet_path):
                    try:
                        temp_df = pd.read_parquet(parquet_path, engine='pyarrow')
                    except Exception as e:
                        st.warning(f"⚠️ Parquet 파일 로드 중 에러 발생 ({parquet_path}): {e}")
                        continue
                        
                # 3. Parquet가 없고 기존 CSV 파일만 있는 경우
                    ...
                    try:
                        ...
                        st.warning(f"⚠️ CSV 파일 로드 중 에러 발생 ({file_path}): {e}")
                        continue
                else:
                    # 둘 다 없는 경우 건너뜀
                    continue

                # 4. 공통 전처리: 로드된 데이터가 비어있지 않고 'text' 컬럼이 있다면 결측치 제거
                if not temp_df.empty and 'text' in temp_df.columns:
                    temp_df = temp_df.dropna(subset=['text']).reset_index(drop=True)
